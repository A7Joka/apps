...
public class ScoreActivity extends AppCompatActivity {
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		_req_request_listener = new RequestNetwork.RequestListener() {
			...
			public void onResponse(String _param1, String _param2, HashMap<String, Object> _param3) {
				...
				if (_response.contains("شعار ")) {
					match = _response.substring((int)(_response.indexOf("<div class=\"row trow py-3\">")), (int)(_response.indexOf("</div><ul class")));
					_GetSource(match, 3, "<div class=\"goals_count cell col-2 p-0 my-auto\">", "</div>");
					_GetSource(match, 2, "<div class=\"team badge\"> ", "</div>");
					_GetSource(match, 1, "<div class=\"cover\">", "</div>");
					_GetSource(match, 0, "<div class=\"rank cell col-1 p-0 my-auto\">", "</div>");
				}
				else {
	...
	private void initializeLogic() {
		...
		textview11.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/neosansarabic.ttf"), 1);
		textview8.setText("ترتيب هدافين ".concat(getIntent().getStringExtra("name")));
	...
	public void _GetSource(final String _Source, final double _type, final String _StartingKey, final String _EndingKey) {
		...
		for(int _repeat21 = 0; _repeat21 < (int)(_Source.length()); _repeat21++) {
			if (_Source.substring((int)(pos1), (int)(pos + 1)).contains(_EndingKey)) {
				if (_type == 3) {
					key5.add(_Source.substring((int)(pos1), (int)(pos)).replace("span", "").replace("/", "").replace("<", "").replace(">", "").replace("div", ""));
				}
				else {
					if (_type == 2) {
						html1 = _Source.substring((int)(pos1), (int)(pos));
						Document docc = Jsoup.parse(html1);
						key3.add(docc.getElementsByTag("img").attr("src"));
						key4.add(docc.getElementsByTag("img").attr("alt").replace("شعار ", ""));
					}
					else {
						if (_type == 1) {
							...
							key1.add(doc.getElementsByTag("img").attr("alt"));
						}
						else {
							number.add(_Source.substring((int)(pos1), (int)(pos)).replace("/", "").replace("<", "").replace(">", "").replace("div", ""));
						}
					}
				}
	...
	public void _Setting() {
		...
		if (settings.getString("language", "").equals("AR")) {
			...
			textview9.setText("Player");
			textview11.setText("Goals");
			textview1.setText("Sorry, Championship Did Not Start");
		}
	}
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			View _view = _v;
			if (_view == null) {
				_view = _inflater.inflate(R.layout.scores, null);
			}
			...
			textview1.setText(number.get((int)(_position)));
			Glide.with(getApplicationContext()).load(Uri.parse(list_data.get((int)_position).get("img").toString())).into(imageview1);
			textview2.setText(key1.get((int)(_position)));
			Glide.with(getApplicationContext()).load(Uri.parse(key3.get((int)(_position)))).into(imageview2);
			textview3.setText(key4.get((int)(_position)));
			textview4.setText(key5.get((int)(_position)));
			if (settings.getString("theme", "").equals("Light")) {
				if ((_position % 2) == 0) {
				...
				linear5.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)10, 0xFF212121));
				textview4.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)90, 0xFF212121));
				_AddCardView(linear6, 0, 200, 5, 5, true, "#212121");
			}
			else {
				if ((_position % 2) == 0) {
				...
				linear5.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)10, 0xFF9E9E9E));
				textview4.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b) { this.setCornerRadius(a); this.setColor(b); return this; } }.getIns((int)90, 0xFF9E9E9E));
				_AddCardView(linear6, 0, 200, 5, 5, true, "#9E9E9E");
			}
